...
%For comparison, ultra-high vacuum chambers built for the most delicate physics experiments have about~\SI{100}{particles} per cubic centimeter~\autocite{gabrielse1990thousandfold}.
...
The Herschel Space Observatory~\autocite{AA_518_L1}
...
are
the Infrared Space Observatory\index{ISO} (ISO)~\autocite{isoHandbook1}
and
Spitzer\index{Spitzer}~\autocite{werner2004spitzer}
...
The camera of PACS (see~\cref{fig:m51_pacs_composite}) revealed the ubiquity of the filamentary structure of star forming regions~\autocite{2010A&A...518L.100M}.
...
of a new population of ``warm dark'' interstellar molecular clouds devoid of~\ce{CO}~\autocite{langer2010c+},
...
\Cref{sec:hifi} begins with a general introduction to the principle of heterodyne detection,
which provides a context to the description of HIFI's hardware that follows.
\Cref{sec:coherence} explains the physics that relates HIFI's optical elements to electromagnetic interferences.
...
As shown by~\cref{eq:sideband_convolution,fig:heterodyne_principle}, given a single spectrum and no a-priori knowledge about the sky, it is impossible to tell whether a feature seen on a spectrum comes from the LSB, from the USB, or from both and in which proportion.
...
\Cref{fig:sideband_ratio} presents a summary of these SBR measurements~\autocite{higgins2010calibration,higgins2014effect}.
