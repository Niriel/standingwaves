...
%=============================================================================
\subsection{Analysis layout}

The analysis performed in this chapter can be summarized by the diagram of~\cref{fig:analysis_flow}.
The four main blocks are data reduction, noise modeling, line analysis, and astronomical modeling.
...
\begin{figure}
    ...
    \begin{tikzpicture}[
            ...
            shl/.style={xshift=-4pt},
            shr/.style={xshift= 4pt},
            shd/.style={yshift=-4pt},
            shu/.style={yshift= 4pt},
            ->,
        ]
        %----- Beginning of matrix.
        \matrix (m)[
            matrix of nodes,
            every odd row/.style={nodes=data},
            every even row/.style={nodes=action},
            nodes in empty cells=false,
            column sep={1cm,between borders},
            row sep={5mm,between borders},
            outer sep=0,
            inner sep=0,
            ]{
            raw data               &                        \\ %  1
            ...
            spectra                &                        \\ %  3
                                   & model noise            \\ %  4
                                   & noise model            \\ %  5
                                   & synthetize spectra     \\ %  6
                                   & spectra                \\ %  7
            fit profiles           & fit profiles           \\ %  8
            integrated intensities & integrated intensities \\ %  9
            divide                 & divide                 \\ % 10
            line ratios            & line ratios            \\ % 11
            analyze statistics     & analyze statistics     \\ % 12
            total uncertainty      & random uncertainty     \\ % 13
        };
        %----- End of matrix.
        \path (m-1-1.north) +(0,0.5) node[data,anchor=south] (input)
        ...
        \path (m.south) + (0,-0.5)
            node[
                ...
                anchor=north,
                matrix of nodes,
                every even row/.style={nodes=data},
                every odd row/.style={nodes=action},
                nodes in empty cells=false,
                column sep={1cm,between borders},
                row sep={5mm,between borders},
                outer sep=0,
                ...
                combine\\
                systematic uncertainty on line ratio\\
                \radex \\
                systematic uncertainty on physical parameters\\
            };
        %----- Frames.
        {[on background layer,
          every node/.style=frame]
            \node[fit=(m-2-1)] {};
            \node[fit=(m-4-2) (m-5-2) (m-6-2)] {};
            \node[fit=(m-8-1) (m-9-1) (m-10-1) (m-11-1) (m-12-1)
                      (m-8-2) (m-9-2) (m-10-2) (m-11-2) (m-12-2)
                      (n-1-1)] {};
            \node[fit=(n-3-1)] {};
        }
        %----- Edges.
        {[on background layer]
            \draw ([shl]input.south) -- ([shl]m-2-1.north);
            \draw ([shr]input.south) -- ([shr]m-2-1.north);
            %
            \draw ([shl]m-2-1.south) -- ([shl]m-8-1.north);
            \draw ([shr]m-2-1.south) -- ([shr]m-8-1.north);
            \draw ([shl]m-8-1.south) -- ([shl]m-10-1.north);
            \draw ([shr]m-8-1.south) -- ([shr]m-10-1.north);
            %
            \draw ([shl]m-2-1.south) to[out=south,in=west] ([shd]m-4-2.west);
            \draw ([shr]m-2-1.south) to[out=south,in=west] ([shu]m-4-2.west);
            %
            \draw ([shl]m-4-2.south) -- ([shl]m-6-2.north);
            \draw ([shr]m-4-2.south) -- ([shr]m-6-2.north);
            \draw ([shl]m-6-2.south) -- ([shl]m-8-2.north);
            \draw ([shr]m-6-2.south) -- ([shr]m-8-2.north);
            \draw ([shl]m-8-2.south) -- ([shl]m-10-2.north);
            \draw ([shr]m-8-2.south) -- ([shr]m-10-2.north);
            %
            \draw (m-10-1) -- (m-12-1);
            \draw (m-10-2) -- (m-12-2);
            \draw (m-12-1) |- (n-1-1);
            \draw (m-12-2) |- (n-1-1);
            \draw (n-1-1) -- (n-3-1);
            \draw (n-3-1) -- (n-4-1.south) -- +(0,-0.2);
            %
          }
    \end{tikzpicture}
    \caption{
        Flow of the data analysis performed in this chapter.
        First, the raw data for both transitions is reduced into usable spectra.
        Second, we analyze the properties of the noise of these spectra in order to create synthetic spectra, which are affected by noise but not by interferences.
        Third, we apply the same analysis to both the real and the synthetic spectra, which yields the total uncertainty on the line ratio and that due to random errors only.
        From these two values we recover the line ratio uncertainty caused by systematic errors.
        Finally, we translate this line ratio uncertainty into an uncertainty on the physical parameters of S140 IRS1.
    }
...
By mis-tuning the diplexer, we do modulate the interferences, but we also deteriorate the sideband gain ratio in a way that is irrelevant for our study.
This section describes how we correct for this degradation of the sideband gain ratio.
...
This is illustrated in~\cref{fig:diplexer_0503}, for which the diplexer actuator current of~\SI{0.503}{\milli\ampere} for a LO~frequency of~\SI{914.4045}{\giga\hertz} results in an arm length mis-tuned by \SI{158}{\micro\meter}, approximately half a wavelength (the details of this calculation are in~\cref{sec:annex_a}).
Consequently, the gain is not symmetrical (top plot) and the sideband gain ratio does not equal \num{0.5} everywhere.
This distorts the line profile and must be corrected.

\begin{figure}
    \centering
    \includegraphics{diplexer_coupling_0503}
    \caption*{
        Gain of the LO and sky signals for an ideal, albeit mis-tuned, diplexer.
    ...
    \caption*{
        Sideband gain ratio of the sky signal for a mis-tuned ideal diplexer.
    ...
    \caption{
        Model of the coupling and the sideband gain ratio for a mis-tuned diplexer.
...
Without loss of generality, we may assume $R_L = R_U = 0$: there is no emission from the Off.

%...................................................................
\paragraph{Step 1: find the continuum.}
We assume that parts of the spectrum show only continuum emission and no line.
Furthermowe, we assume that the continuum is emitted by a homogeneous black body:
its amplitude (in units of temperature) is constant.
...
\end{equation}
In this equation, $G_L$ and $G_U$ are known: we can use a diplexer model (see~\cref{eq:diplexer_coupling_model} in \cref{sec:annex_a}) to calculate these terms from the known diplexer arm displacement.
...
With $B$ the continuum intensity that we just calculated and $J$ the intensity of the line, we have
\begin{equation}
    T_\text{mb}
    =
    \Delta T
...
The rest of this chapter uses the $T_\text{mb}''$ scale.

%-----------------------------------------------------------------------------
\subsubsection{Result of the diplexer mis-tuning correction}
The 190 spectra of our S140 observation were corrected for diplexer mis-tuning.
\Cref{fig:diplexer_correction,tab:diplexer_correction} illustrate the line profiles and peak intensities before and after correction.
The the spread of the peak intensity, listed in~\cref{tab:diplexer_correction}, is considerably reduced by correcting for the diplexer mis-tuning.
...
\begin{figure}
    ...
    \begin{tabular}{c}
        ...
        \\
        \footnotesize{\transition{CO}{8}{7} profiles before (top) and after (bottom) correction.}
        \\
        \\
        \includegraphics{correction_98}
        \\
        \footnotesize{\transition{CO}{9}{8} profiles before (top) and after (bottom) correction.}
        \\
    \end{tabular}
    \caption{
        This figure illustrates how the profile of spectral lines measured by HIFI is impacted by
        a mismatch between LO frequency and diplexer tuning.
        Each plot shows the superposition of 95 spectra of S140 IRS1.
    }
...
Each channel of a HIFI spectrum is a measurement $X_\text{m}$ of a physical quantity whose true value $X_\text{t}$ is unknown.
The difference $\Delta X = X_\text{m} - X_\text{t}$, unknown as well, is called ``measurement error''.
As detailed in~\vref{sec:error_budget} (\nameref{sec:error_budget}),
this error is a function of many parameters such as thermal background radiation, mixer intrinsic sideband ratio, optical interferences, electrical interferences%
\footnote{
    Electromagnetic interferences are not limited to the optical part (between the sky and the mixer) of HIFI, they can also occur in the waveguides of its electronics (between the mixer and the detector).
    These ``electrical interferences'' are not an issue in Band~3 or 4 but often dominate in Band~6 and 7~\autocite{higgins2011}.
}%
, LO purity, or LO stability.


The total error $\Delta X$ on the measurement of a variable $X$ results from systematic and statistical errors $\Delta X_\text{systematic}$ and $\Delta X_\text{statistical}$.
...
\begin{equation}
    ...
    =
    \sqrt{
        \Delta X_\text{systematic}^2
        +
        \Delta X_\text{statistical}^2
    }
\end{equation}
This equation holds because systematic and statistical errors are independant and uncorrelated.

In the case of HIFI, the effect of electromagnetic interferences is sytematic,
while the thermal background radiation is random in nature.
...
To obtain almost noiseless spectra, we average the 95~spectra of each transition.
These averaged spectra are shown in~\cref{fig:corrected_averaged}.
...
\begin{figure}
    ...
    \begin{subfigure}[b]{\textwidth}
        ...
        \label{fig:corrected_averaged_87}
    \end{subfigure}
    \\
    \begin{subfigure}[b]{\textwidth}
        \centering
        ...
        \label{fig:corrected_averaged_98}
    \end{subfigure}
...
\begin{figure}[p]
    ...
    \begin{subfigure}[b]{\textwidth}
        ...
        \label{fig:noise_model_98_correctedavg}
    \end{subfigure}
    \\
    \bigskip
    \begin{subfigure}[b]{\textwidth}
        ...
        \label{fig:noise_model_98_original}
    \end{subfigure}
    \\
    \bigskip
    \begin{subfigure}[b]{\textwidth}
        ...
        \label{fig:noise_model_98_noisy}
    \end{subfigure}
...
\begin{figure}
    ...
    \begin{subfigure}[b]{\textwidth}
        ...
        \label{fig:spectra_transitions_87}
    \end{subfigure}
    \\
    \bigskip
    \begin{subfigure}[b]{\textwidth}
        ...
        \label{fig:spectra_transitions_98}
    \end{subfigure}
    \caption{
...
Obviously the LO~frequency influences the sideband gain ratio in ways that our diplexer mis-tuning correction does not predict.
