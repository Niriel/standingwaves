...
\author{Bertrand~Delforge,~\IEEEmembership{Member,~IEEE,}
        John~Pearson,~\IEEEmembership{Fellow,~OSA,}
        Peter~Roelfsema,~\IEEEmembership{Fellow,~OSA,}
        Marc~Verheijen,~\IEEEmembership{Fellow,~OSA,}
        and~Willem~Jellema,~\IEEEmembership{Life~Fellow,~IEEE}% <-this % stops a space
...
{Delforge \MakeLowercase{\textit{et al.}}: Understanding Standing Waes by Network Modeling}
...
\begin{figure}
    ...
    The standard deviation scatter on the line peak is three times greater than the expected noise (inset plots).
    }%
...
\subsubsection{Typical networks.}
...
Wire grid polarizers are much more difficult to model; we adapted the work of \cite{houde_2001} to out matrix format.
...
\begin{figure}
    ...
    \caption{Calibration scheme that uses the sideband ratio provided by the model.}
...
Standing waves, being frequency-dependent, modulate the lower and upper sidebands differently, resulting in an imbalanced coupling ratio between the two sidebands during the folding by the mixer, see figure~\ref{fig:sbr}.
...
\begin{figure}
    ...
    The sideband ratio varies quickly over the intermediate frequency (IF), as several standing wave patterns beat and modulate each other.
    The \SI{620}{\mega\hertz}-wide periodic modulation comes from the standing wave between the mixers and the closest roof-top mirrors of the diplexer assemblies.  The wide envelop results from the fact that the diplexer tuning is optimal at the center of the IF and degrades on the edges.  The short-period features correspond to the cavities formed by the mixers and the local oscilator (\SI{100}{\mega\hertz}), cold black body (\SI{60}{\mega\hertz}) or hot black body (\SI{75}{\mega\hertz}) (revealed by taking Fourier transforms of these spectra).}%
    \label{fig:sbr}
\end{figure}
...
\begin{IEEEbiography}{Bertrand Delforge}
Biography text here.
\end{IEEEbiography}
