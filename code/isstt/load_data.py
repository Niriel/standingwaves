...
#     noises = []
    ...
    for filename in filenames:
        ...
        if len(x) <= 1:
            continue
        before = np.logical_and(x > 1036.50, x < 1037.00)
        after = np.logical_and(x > 1037.30, x < 1037.80)
        noline = np.logical_or(before, after)
        baseline = np.average(y[noline])
        no_baseline = y - baseline
        continuum_idx = y - baseline < .5
        los.append(lo)
#         maxes.append(np.max(smooth(no_baseline)))
